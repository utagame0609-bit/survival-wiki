export type SoundCategoryV1 = 'system' | 'screen' | 'action' | 'wiki';

export type SoundEffectV1 = {
  id: string;
  name: string;
  nameJa: string;
...
/** 音源候補v1のSE仕様をアプリ側へ独立保持する定義表。既存sound.tsには未接続。 */
export const SOUND_EFFECTS_V1: SoundEffectV1[] = [
...
export const getSoundEffectV1 = (id: string): SoundEffectV1 | undefined => SOUND_EFFECTS_V1.find((effect) => effect.id === id);
