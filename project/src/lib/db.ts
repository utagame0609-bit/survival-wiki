...
export async function fetchWorlds(gameId: string): Promise<WorldWithMembers[]> {
  ...
  if (error) throw error;
  return (data ?? []).map((w) => ({ ...w, members: w.world_members ?? [] }));
}

export async function fetchLatestLocationDates(worldIds: string[]): Promise<Record<string, string | null>> {
  if (worldIds.length === 0) return {};
  const { data, error } = await supabase.from('locations').select('world_id, created_at').in('world_id', worldIds).order('created_at', { ascending: false });
  if (error) throw error;
  const latest: Record<string, string | null> = {};
  for (const worldId of worldIds) latest[worldId] = null;
  for (const row of data ?? []) {
    if (latest[row.world_id] === null) latest[row.world_id] = row.created_at;
  }
  return latest;
}

export async function fetchWorld(id: string): Promise<WorldWithMembers | null> {
  const { data, error } = await supabase.from('worlds').select('*, world_members(*)').eq('id', id).maybeSingle();
  if (error) throw error;
  if (!data) return null;
...
export async function fetchLocations(worldId: string): Promise<LocationWithPhotos[]> {
  const { data, error } = await supabase.from('locations').select('*, location_photos(*)').eq('world_id', worldId).order('created_at', { ascending: false });
  if (error) throw error;
  const locations = (data ?? []) as (Location & { location_photos: LocationPhoto[] })[];
  console.debug('[Survival Wiki] fetched location photos', locations.map((location) => ({ location: location.name, photos: (location.location_photos ?? []).map((photo) => ({ id: photo.id, path: photo.storage_path, isMain: photo.is_main })) })));
  const { data: lmData, error: lmErr } = await supabase.from('location_members').select('location_id, world_members(*)').in('location_id', locations.map((l) => l.id));
...
export async function createLocation(worldId: string, input: { name: string; x: number; y: number; z: number; detail_memo: string; created_at: string; member_ids: string[] }): Promise<Location> {
  ...
  if (input.member_ids.length > 0) {
    const rows = input.member_ids.map((mid) => ({ location_id: loc.id, member_id: mid }));
...
export async function deleteLocation(id: string): Promise<void> {
  const { data: photos, error: pErr } = await supabase.from('location_photos').select('storage_path').eq('location_id', id);
  if (pErr) throw pErr;
...
export async function fetchWikiArticle(worldId: string, style: string): Promise<WikiArticle | null> {
  const { data, error } = await supabase.from('wiki_articles').select('*').eq('world_id', worldId).eq('style', style).maybeSingle();
  if (error) throw error;
...
export async function saveWikiArticle(worldId: string, style: string, content: string): Promise<WikiArticle> {
  ...
  if (existing) {
    const { data, error } = await supabase.from('wiki_articles').update({ content, generated_at: new Date().toISOString() }).eq('id', existing.id).select().single();
    if (error) throw error;
    return data as WikiArticle;
  }
  const { data, error } = await supabase.from('wiki_articles').insert({ world_id: worldId, style, content, generated_at: new Date().toISOString() }).select().single();
  if (error) throw error;
  return data as WikiArticle;
}

export async function resetWikiArticle(worldId: string, style: string): Promise<void> {
  const { error } = await supabase.from('wiki_articles').delete().eq('world_id', worldId).eq('style', style);
  if (error) throw error;
}
