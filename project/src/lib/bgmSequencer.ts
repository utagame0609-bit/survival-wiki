...
class BgmSequencer {
  ...
  public play(){if(this.isPlaying)return;const ctx=soundEngine.init();this.isPlaying=true;this.nextStepTime=ctx.currentTime+.05;this.currentStep=0;this.timerId=window.setInterval(()=>this.scheduler(),this.lookahead);this.scheduler();}
  ...
  private scheduler(){if(!this.isPlaying)return;const ctx=soundEngine.getContext();while(this.nextStepTime<ctx.currentTime+this.scheduleAheadTime){this.scheduleStep(this.currentStep,this.nextStepTime);this.nextStepTime+=60/(this.tempo*4);this.currentStep=(this.currentStep+1)%this.totalSteps;}}
  private scheduleStep(step:number,time:number){this.state=getBgmChannelSettings();const d=60/(this.tempo*4);if(this.state.lead)this.leadNotes.filter(n=>n.time===step).forEach(n=>this.playLead(n.freq,time,n.duration*d,n.velocity??.8));if(this.state.harmony)this.harmonyNotes.filter(n=>n.time===step).forEach(n=>this.playHarmony(n.freq,time,n.duration*d,n.velocity??.5));if(this.state.bass)this.bassNotes.filter(n=>n.time===step).forEach(n=>this.playBass(n.freq,time,n.duration*d,n.velocity??.8));if(this.state.drums)this.drumNotes.filter(n=>n.time===step).forEach(n=>this.playDrum(n.type,time,n.velocity??.7));}
