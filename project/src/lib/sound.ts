...
export function playSound(sound: SoundType): void {
  if (!enabled) return;
  if (sound === 'confirm') { playV1Confirm(); return; }
  if (sound === 'cancel') { playV1Cancel(); return; }
...
export const playNewRecordSound = () => {
  ...
  if (context.state === 'suspended') void context.resume();
  const now = context.currentTime;
  [1318.51, 1760.0, 2637.02].forEach((freq, idx) => {
    ...
    osc.start(noteTime); osc.stop(noteTime + (idx === 2 ? 0.48 : 0.15));
  });
  ...
  sparkleOsc.connect(sparkleGain); sparkleGain.connect(masterGain!);
  sparkleOsc.start(now + 0.06); sparkleOsc.stop(now + 0.42);
};
...
export const playChestOpenSound = () => {
  ...
  for (let i = 0; i < noiseBuffer.length; i += 1) noiseData[i] = (Math.random() * 2 - 1) * Math.exp(-i / (context.sampleRate * 0.008));
  const noiseNode = context.createBufferSource();
  noiseNode.buffer = noiseBuffer;
  const noiseFilter = context.createBiquadFilter();
  noiseFilter.type = 'bandpass'; noiseFilter.frequency.setValueAtTime(650, now); noiseFilter.Q.setValueAtTime(3, now);
  const noiseGain = context.createGain(); noiseGain.gain.setValueAtTime(0.3, now);
  noiseNode.connect(noiseFilter); noiseFilter.connect(noiseGain); noiseGain.connect(masterGain!); noiseNode.start(now);
  const bodyOsc = context.createOscillator(); const bodyGain = context.createGain();
  bodyOsc.type = 'triangle'; bodyOsc.frequency.setValueAtTime(260, now); bodyOsc.frequency.exponentialRampToValueAtTime(130, now + 0.06);
  bodyGain.gain.setValueAtTime(0.32, now); bodyGain.gain.exponentialRampToValueAtTime(0.001, now + 0.07);
  bodyOsc.connect(bodyGain); bodyGain.connect(masterGain!); bodyOsc.start(now); bodyOsc.stop(now + 0.08);
  const chimeOsc = context.createOscillator(); const chimeGain = context.createGain();
  chimeOsc.type = 'square'; chimeOsc.frequency.setValueAtTime(523.25, now + 0.025); chimeOsc.frequency.exponentialRampToValueAtTime(1046.5, now + 0.07);
  chimeGain.gain.setValueAtTime(0.001, now + 0.025); chimeGain.gain.linearRampToValueAtTime(0.25, now + 0.035); chimeGain.gain.exponentialRampToValueAtTime(0.001, now + 0.18);
  chimeOsc.connect(chimeGain); chimeGain.connect(masterGain!); chimeOsc.start(now + 0.025); chimeOsc.stop(now + 0.2);
};
...
function playCursorMoveSoundDirect(): void {
  ...
  const airFilter = context.createBiquadFilter(); const airGain = context.createGain();
  osc.type = 'square'; osc.frequency.setValueAtTime(1320, now); osc.frequency.exponentialRampToValueAtTime(1980, now + 0.045);
  gain.gain.setValueAtTime(0.0001, now); gain.gain.linearRampToValueAtTime(0.16, now + 0.004); gain.gain.exponentialRampToValueAtTime(0.0001, now + 0.045);
  ...
  const airSource = context.createBufferSource(); airSource.buffer = airBuffer;
  airFilter.type = 'highshelf'; airFilter.frequency.setValueAtTime(3000, now); airFilter.gain.setValueAtTime(5, now);
  airGain.gain.setValueAtTime(0.045, now); airGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.045);
  airSource.connect(airFilter); airFilter.connect(airGain); airGain.connect(masterGain);
  osc.start(now); osc.stop(now + 0.055); airSource.start(now); airSource.stop(now + 0.045);
}
