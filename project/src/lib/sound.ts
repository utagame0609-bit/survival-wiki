...
function hiss(c: AudioContext, time: number, duration: number, volume: number, type: BiquadFilterType, frequency: number): void {
  ...
  gain.gain.setValueAtTime(volume * SOURCE_LEVEL, time);
  gain.gain.exponentialRampToValueAtTime(0.0001, time + duration);
...
function playV2Sound(sound: SoundType): void {
  ...
  switch (sound) {
    ...
    case 'delete': tone(c, 116, t, 0.28, 0.18, 'sawtooth'); tone(c, 123, t, 0.28, 0.14, 'square'); tone(c, 65, t, 0.16, 0.2, 'triangle', 50); hiss(c, t, 0.28, 0.05, 'lowpass', 1200); return;
    case 'tabSwitch': tone(c, 950, t, 0.055, 0.15, 'square', 1600); hiss(c, t, 0.05, 0.07, 'bandpass', 3200); return;
    case 'modalOpen':
    case 'modalClose': tone(c, 480, t, 0.12, 0.22, 'sine', 110); return;
    case 'footstep': tone(c, 140, t, 0.065, 0.13, 'triangle', 65); hiss(c, t, 0.055, 0.025, 'lowpass', 1800); return;
    case 'hover': tone(c, 2200, t, 0.022, 0.055, 'square'); return;
    case 'cardOpen': tone(c, 520, t, 0.095, 0.12, 'square', 1480); hiss(c, t, 0.08, 0.025, 'highpass', 4200); return;
    case 'cardClose': tone(c, 980, t, 0.085, 0.11, 'triangle', 320); return;
    case 'add': tone(c, 783.99, t, 0.09, 0.12, 'square'); tone(c, 1046.5, t + 0.04, 0.09, 0.12, 'square'); tone(c, 1318.51, t + 0.08, 0.09, 0.12, 'square'); return;
    ...
    case 'chestClose': tone(c, 1400, t, 0.14, 0.16, 'square', 700); tone(c, 180, t, 0.14, 0.09, 'triangle', 120); hiss(c, t, 0.08, 0.03, 'lowpass', 900); return;
    case 'screenTransition': hiss(c, t, 0.36, 0.1, 'bandpass', 4500); tone(c, 85, t, 0.36, 0.14, 'sine', 55); return;
    case 'notification': tone(c, 1046.5, t, 0.16, 0.1, 'sine'); tone(c, 1318.5, t + 0.06, 0.1, 0.08, 'square'); return;
    case 'inputFocus': tone(c, 1600, t, 0.028, 0.08, 'sine', 800); return;
    default: return;
  }
}
...
export const playNewRecordSound = () => {
  ...
  if (!c || !masterGain || !enabled) return;
  const t = c.currentTime;
  [1318.51, 1760, 2637.02].forEach((frequency, i) => tone(c, frequency, t + i * 0.03, i === 2 ? 0.45 : 0.12, 0.18, 'square'));
};
...
function createWikipediaBgm(c: AudioContext): ActiveNpcBgm {
  ...
  const interval = window.setInterval(() => {
    ...
    if (m) tone(c, m, t, stepTime * 0.9, 0.04, 'triangle');
    if (b) tone(c, b, t, stepTime * 0.9, 0.08, 'sine');
    step += 1;
  }, stepTime * 1000);
  return {
...
function createScpBgm(c: AudioContext): ActiveNpcBgm {
  ...
  const bassNotes = [55, 55, 65.41, 55, 49, 55, 73.42, 65.41];
  const interval = window.setInterval(() => {
    if (!audioContext || audioContext.state === 'suspended') return;
    const t = audioContext.currentTime;
    const note = bassNotes[step % bassNotes.length];
    tone(c, note, t, stepTime * 0.8, 0.08, 'sawtooth');
    step += 1;
  }, stepTime * 1000);
  return {
    id: 'npc_bgm_scp',
...
function createAncientBgm(c: AudioContext): ActiveNpcBgm {
  ...
  const interval = window.setInterval(() => {
    ...
    tone(c, note, t, stepTime * 0.85, 0.045, 'triangle');
    step += 1;
  }, stepTime * 1000);
  return {
    id: 'npc_bgm_ancient',
...
function playNpcBgm(id: ActiveNpcBgm['id']): void {
  const c = getAudioContext();
  if (!c || !enabled) return;
  stopNpcBgmInternal();
  activeNpcBgm = id === 'npc_bgm_wikipedia'
    ? createWikipediaBgm(c)
    : id === 'npc_bgm_scp'
      ? createScpBgm(c)
      : createAncientBgm(c);
}
...
export function setSoundVolume(value: number): void {
  soundVolume = Math.min(100, Math.max(0, value));
  if (typeof window !== 'undefined') window.localStorage.setItem(SOUND_VOLUME_KEY, String(soundVolume));
  if (masterGain && audioContext) masterGain.gain.setTargetAtTime(getMasterGainValue(), audioContext.currentTime, 0.015);
}
...
export function playNpcBgmWikipedia(): void {
  playNpcBgm('npc_bgm_wikipedia');
}

export function playNpcBgmScp(): void {
  playNpcBgm('npc_bgm_scp');
}

export function playNpcBgmAncient(): void {
  playNpcBgm('npc_bgm_ancient');
}
