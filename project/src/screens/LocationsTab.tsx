  ...
  return (
    <div className="min-h-full bg-[#0a1120] text-[#e2e8f0] px-4 sm:px-6 py-6 max-w-4xl mx-auto font-dot">
      <div className="mb-6 space-y-3">
        <button onClick={() => { playConfirmSound(); setMode({ type: 'create' }); }} className="group w-full flex items-center justify-center gap-3 px-5 py-4 rounded-sm bg-[#ffb000] text-[#0a1120] font-black text-sm sm:text-base border-2 border-[#ffb000] shadow-[0_0_20px_rgba(255,176,0,0.25)] hover:bg-white hover:border-white hover:text-[#0a1120] active:scale-[0.99] transition-all">
        ...
        <div className="relative">
          ...
          <input type="search" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="探検ログ・地名や調査メモを検索..." aria-label="ロケーションを検索" className="w-full pl-10 pr-4 py-3 rounded-sm bg-[#0d1627] border-2 border-[#334155] text-xs sm:text-sm text-[#e2e8f0] placeholder:text-zinc-500 outline-none transition-all focus:border-[#32cd32] focus:shadow-[0_0_15px_rgba(50,205,50,0.2)]" />
        </div>
      </div>
        ...
        <>
          <div className="flex justify-between items-center px-1 mb-4 text-xs text-zinc-400 font-mono gap-3">
            <div className="flex items-center gap-2 min-w-0">
              <button type="button" onClick={() => { playChestOpenSound(); setCollectionOpen(true); }} aria-label="コレクションを開く" title="宝箱・写真コレクション" className="command-btn shrink-0 px-3 py-2 rounded-sm bg-[#1a2333] text-[#ffb000] hover:text-[#ffb000] active:scale-[0.96] transition-all flex items-center gap-2">
            ...
            <button type="button" onClick={() => { playToggleSound(); setSortOrder((current) => current === 'asc' ? 'desc' : 'asc'); }} className="command-btn shrink-0 inline-flex items-center gap-1.5 px-3 py-2 rounded-sm bg-[#1a2333] text-zinc-300 hover:text-[#ffb000] text-[11px]" aria-label="ロケーションの並び順を変更">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm font-dot" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) { playCancelSound(); setDeleteTarget(null); } }}>
          <div role="dialog" aria-modal="true" aria-labelledby="delete-location-title" className="w-full max-w-md overflow-hidden rounded-sm bg-[#0a1120] border-4 border-double border-red-600 shadow-[0_0_30px_rgba(239,68,68,0.3)]">
            <div className="px-5 pt-6 pb-5 text-center">
              ...
              <p className="mt-2 text-sm text-[#ffb000] font-bold break-words">「{deleteTarget.name}」</p>
              <p className="mt-3 text-xs leading-5 text-zinc-400 font-mono">この操作は冒険の書から元に戻せません。<br />保存された写真と座標データもすべて破棄されます。</p>
            </div>
            <div className="grid grid-cols-2 gap-3 px-5 pb-5">
              <button type="button" onClick={() => { playCancelSound(); setDeleteTarget(null); }} className="command-btn py-2.5 bg-[#1a2333] text-zinc-300 hover:text-white text-xs font-bold font-mono"><X className="w-4 h-4 inline mr-1" />キャンセル</button>
              <button type="button" onClick={confirmDelete} className="py-2.5 rounded-sm bg-red-800 border-2 border-red-500 text-white hover:bg-red-700 active:scale-[0.98] transition-all text-xs font-bold font-mono shadow-[0_0_15px_rgba(239,68,68,0.4)]"><Trash2 className="w-4 h-4 inline mr-1" />抹消する</button>
            </div>
          </div>
        </div>
      )}
      ...
      {mode.type !== 'list' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-dot" onMouseDown={(event) => { if (event.target === event.currentTarget) { playCancelSound(); closeModal(); } }}>
          <div className="w-full max-w-lg max-h-[calc(100vh-2rem)] overflow-y-auto rounded-sm bg-[#0a1120] border-4 border-double border-[#ffb000] shadow-[0_0_30px_rgba(255,176,0,0.25)] motion-safe:animate-[modal-enter_180ms_cubic-bezier(.22,.8,.35,1)]">
            <div className="flex items-center justify-between px-5 py-3.5 border-b-2 border-[#1a2333] bg-[#0d1627]">
              <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-[#ffb000] shadow-[0_0_8px_#ffb000]" /><h2 className="text-base font-bold text-[#ffb000] tracking-wide">{mode.type === 'edit' ? 'ロケーション情報の更新' : '新たな拠点の記録'}</h2></div>
              <button type="button" onClick={() => { playCancelSound(); closeModal(); }} disabled={saving} aria-label="閉じる" className="p-1 rounded-sm text-zinc-400 hover:bg-[#1a2333] hover:text-[#ffb000] transition-colors border border-transparent hover:border-[#334155]"><X className="w-5 h-5" /></button>
            </div>
    ...
    <div className="group relative overflow-hidden rounded-sm bg-[#0d1627] border-2 border-[#1a2333] shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:border-[#ffb000] hover:shadow-[0_0_20px_rgba(255,176,0,0.2)]">
      <button onClick={onToggle} className="w-full text-left active:scale-[0.99] transition-transform">
        <div className="flex gap-3.5 sm:gap-4 p-3.5 sm:p-4 pr-5 items-center">
          {mainPhoto ? (
            <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-sm overflow-hidden bg-[#050a14] border-2 border-[#334155] group-hover:border-[#ffb000] transition-colors shrink-0"><PhotoImage storagePath={mainPhoto.storage_path} alt={loc.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" /></div>
          ) : (
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-sm bg-[#050a14] border-2 border-[#334155] group-hover:border-[#ffb000] flex flex-col items-center justify-center gap-1 shrink-0 transition-colors"><MapPin className="w-6 h-6 text-[#ffb000]/60 group-hover:text-[#ffb000] transition-colors" /><span className="text-[9px] text-zinc-500 font-mono">NO IMAGE</span></div>
          )}
          <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5">
            <div className="flex items-center gap-2 min-w-0">
              {slotNumber !== undefined && <span className="px-2 py-0.5 rounded-sm bg-[#050a14] border border-[#334155] text-[10px] font-mono text-[#ffb000] font-bold shrink-0">SLOT {String(slotNumber).padStart(2, '0')}</span>}
