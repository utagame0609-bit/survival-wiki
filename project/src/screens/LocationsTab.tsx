...
export function LocationsTab({ world, reloadKey, onReload, openLocationId, onOpenLocationHandled }: { world: WorldWithMembers; reloadKey: number; onReload: () => void; openLocationId?: string | null; onOpenLocationHandled?: () => void }) {
  ...
  const handleComplete = async () => {
    try { await load(); setMode({ type: 'list' }); onReload(); }
  ...
  const openCreateModal = () => { playConfirmSound(); setMode({ type: 'create' }); };
  const closeModal = () => { if (saving) return; playModalCloseSound(); setMode({ type: 'list' }); };
  const closeLocationDetail = () => { playModalCloseSound(); setSelectedLocation(null); };
  ...
  const filteredLocations = normalizedQuery ? locations.filter((loc) => loc.name.toLocaleLowerCase().includes(normalizedQuery) || loc.detail_memo?.toLocaleLowerCase().includes(normalizedQuery)) : locations;
  const sortedLocations = [...filteredLocations].sort((a, b) => {
    const aTime = new Date(a.created_at).getTime();
    const bTime = new Date(b.created_at).getTime();
    return sortOrder === 'asc' ? aTime - bTime : bTime - aTime;
  });
  const collectionItems: CollectionItem[] = locations.flatMap((location) => location.photos.map((photo) => ({ location, storagePath: photo.storage_path, createdAt: photo.created_at }))).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(({ location, storagePath }) => ({ location, storagePath }));
    ...
    <div className="space-y-4 sm:space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 items-stretch sm:items-center justify-between bg-[#1e2330] p-3 sm:p-4 border-2 border-[#2d3548] shadow-md">
        <div className="flex-1 flex items-center gap-2 bg-[#12151f] border border-slate-700 px-3.5 py-2.5 focus-within:border-amber-500 min-h-[44px]">
          ...
          />
          {searchQuery && (
            <button type="button" onClick={() => setSearchQuery('')} onMouseEnter={playHoverSound} className="min-h-[32px] min-w-[32px] flex items-center justify-center text-slate-400 hover:text-white" aria-label="検索をクリア">×</button>
          )}
        </div>
      ...
      {!loading && locations.length === 0 && <div className="py-16 text-center border-2 border-dashed border-[#2d3548] bg-[#141824]/60 p-6"><MapPin className="w-12 h-12 mx-auto text-slate-500 mb-3" /><p className="text-base font-bold text-white">ロケーションがありません</p><p className="text-xs sm:text-sm text-slate-400 mt-1">「+ 新規記録」から最初の拠点を登録してください。</p></div>}
      ...
      {selectedLocation && <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-sm text-xs sm:text-sm font-sans">
        ...
        <div className="relative z-10 w-full max-w-lg bg-[#1e2330] border-2 border-amber-500/70 shadow-[0_0_40px_rgba(0,0,0,0.6)] overflow-hidden motion-safe:animate-[modal-enter_180ms_cubic-bezier(.22,.8,.35,1)]">
          <div className="px-4 sm:px-5 py-3.5 bg-[#161a24] border-b-2 border-[#2d3548] flex items-center justify-between"><div className="flex items-center gap-2 min-w-0"><MapPin className="w-5 h-5 text-amber-400 shrink-0" /><h2 className="text-base font-bold text-white truncate">{selectedLocation.name}</h2></div><button type="button" onClick={closeLocationDetail} onMouseEnter={playHoverSound} className="min-h-[36px] min-w-[36px] flex items-center justify-center text-slate-300 hover:text-white text-lg cursor-pointer" aria-label="閉じる">×</button></div>
          <div className="p-4 sm:p-5 space-y-4 max-h-[75vh] overflow-y-auto">
            {(() => { const mainPhoto = selectedLocation.photos.find((p) => p.is_main); return mainPhoto ? <div className="w-full h-36 sm:h-44 overflow-hidden bg-[#12151f] border border-[#2d3548]"><PhotoImage storagePath={mainPhoto.storage_path} alt={selectedLocation.name} className="w-full h-full object-cover pixelated" /></div> : null; })()}
            <div className="p-3.5 bg-[#12151f] border border-slate-700 flex items-center justify-around text-center"><div><div className="text-[10px] text-slate-400 uppercase font-mono font-bold">X COORDINATE</div><div className="text-base font-bold text-emerald-400 font-mono">{selectedLocation.x}</div></div><div className="h-7 w-px bg-slate-700" /><div><div className="text-[10px] text-slate-400 uppercase font-mono font-bold">Y ELEVATION</div><div className="text-base font-bold text-emerald-400 font-mono">{selectedLocation.y}</div></div><div className="h-7 w-px bg-slate-700" /><div><div className="text-[10px] text-slate-400 uppercase font-mono font-bold">Z COORDINATE</div><div className="text-base font-bold text-cyan-300 font-mono">{selectedLocation.z}</div></div></div>
            {selectedLocation.detail_memo && <div className="p-4 bg-[#141824] border border-[#2d3548]"><h4 className="text-xs text-amber-400 font-bold uppercase mb-2">探索・観測メモ</h4><p className="text-slate-100 text-sm leading-relaxed whitespace-pre-wrap">{selectedLocation.detail_memo}</p></div>}
            {selectedLocation.members.length > 0 && <div><h4 className="text-xs text-cyan-400 font-bold uppercase mb-2">同行メンバー</h4><div className="flex flex-wrap gap-2">{selectedLocation.members.map((m) => <span key={m.id} className="px-2.5 py-1 bg-[#12151f] border border-cyan-500/40 text-cyan-300 text-xs font-medium">@{m.name}</span>)}</div></div>}
          ...
          <div className="px-4 sm:px-5 py-3.5 bg-[#161a24] border-t border-[#2d3548] flex items-center justify-end gap-2"><button type="button" onClick={handleDetailEdit} onMouseEnter={playHoverSound} className="min-h-[40px] px-3.5 py-2 bg-[#12151f] text-slate-200 border border-slate-700 hover:border-amber-500 hover:text-amber-400 font-bold text-xs cursor-pointer">編集</button><button type="button" onClick={() => handleDelete(selectedLocation)} onMouseEnter={playHoverSound} className="min-h-[40px] px-3.5 py-2 bg-red-950/50 text-red-300 border border-red-800 hover:bg-red-900/60 font-bold text-xs cursor-pointer">削除</button></div>
        </div>
      </div>}

      {deleteTarget && <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm font-sans" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) { playCancelSound(); setDeleteTarget(null); } }}><div role="dialog" aria-modal="true" aria-labelledby="delete-location-title" className="w-full max-w-md overflow-hidden bg-[#1e2330] border-2 border-red-600 shadow-[0_0_30px_rgba(239,68,68,0.3)]"><div className="px-5 pt-6 pb-5 text-center"><div className="mx-auto mb-4 w-14 h-14 rounded-full bg-red-950/80 border-2 border-red-700 flex items-center justify-center shadow-[0_0_20px_rgba(239,68,68,0.3)]"><AlertTriangle className="w-7 h-7 text-red-400" /></div><h2 id="delete-location-title" className="text-base sm:text-lg font-bold text-red-200 font-mono uppercase">ロケーションを抹消しますか？</h2><p className="mt-2 text-sm text-amber-400 font-bold break-words">「{deleteTarget.name}」</p><p className="mt-3 text-xs leading-5 text-slate-400 font-mono">この操作は冒険の書から元に戻せません。<br />保存された写真と座標データもすべて破棄されます。</p></div><div className="grid grid-cols-2 gap-3 px-5 pb-5"><button type="button" onClick={() => { playCancelSound(); setDeleteTarget(null); }} onMouseEnter={playHoverSound} className="min-h-[40px] bg-[#12151f] border border-slate-700 text-slate-300 hover:text-white text-xs font-bold font-mono"><X className="w-4 h-4 inline mr-1" />キャンセル</button><button type="button" onClick={confirmDelete} onMouseEnter={playHoverSound} className="min-h-[40px] rounded-sm bg-red-800 border-2 border-red-500 text-white hover:bg-red-700 active:scale-[0.98] transition-all text-xs font-bold font-mono shadow-[0_0_15px_rgba(239,68,68,0.4)]"><Trash2 className="w-4 h-4 inline mr-1" />抹消する</button></div></div></div>}

      {mode.type !== 'list' && <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-6 bg-black/85 backdrop-blur-sm font-sans" onMouseDown={(event) => { if (event.target === event.currentTarget) { playCancelSound(); closeModal(); } }}><div className="relative w-full max-w-xl max-h-[92vh] overflow-y-auto bg-[#1e2330] border-2 border-amber-500/70 shadow-[0_0_40px_rgba(0,0,0,0.6)] motion-safe:animate-[modal-enter_180ms_cubic-bezier(.22,.8,.35,1)]"><div className="px-4 sm:px-5 py-3 bg-[#161a24] border-b-2 border-[#2d3548] flex items-center justify-between"><div className="flex items-center gap-2.5"><span className="text-xs px-2.5 py-0.5 border border-amber-500/50 bg-amber-500/15 text-amber-300 font-bold font-mono">{mode.type === 'edit' ? 'EDIT' : 'NEW'}</span><h2 className="text-sm sm:text-base font-bold text-white">{mode.type === 'edit' ? 'ロケーション編集' : '新規拠点記録'}</h2></div><button type="button" onClick={() => { playCancelSound(); closeModal(); }} onMouseEnter={playHoverSound} disabled={saving} className="min-h-[36px] min-w-[36px] flex items-center justify-center text-slate-300 hover:text-white text-lg cursor-pointer" aria-label="閉じる">×</button></div><LocationForm worldId={world.id} members={world.members} editing={mode.type === 'edit' ? mode.location : null} onSave={handleSave} onComplete={handleComplete} onCancel={closeModal} saving={saving} /></div></div>}
