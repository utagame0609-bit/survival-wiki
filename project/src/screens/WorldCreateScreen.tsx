...
export function WorldCreateScreen({ gameId, gameName, worldId, navigate, goBack }: { gameId: string; gameName: string; worldId?: string; navigate: NavigateFn; goBack: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) playSaveSound(); else playNewRecordSound();
      goBack();
    } catch (e) { setError((e as Error).message); } finally { setSaving(false); }
  };

  if (loading) return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-3 sm:p-4"><Spinner label="読み込み中" /></div>;
  ...
  return (
    ...
    >
      <div className="world-edit-modal-panel w-full max-w-lg max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto overflow-x-hidden bg-[#0a1120] border-2 border-amber-500/80 shadow-[0_0_35px_rgba(245,158,11,0.22)] text-slate-100">
        <div className="flex items-center justify-between px-4 sm:px-5 py-3.5 border-b border-slate-800 bg-[#0d1627]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm sm:text-base font-bold text-white">{isEdit ? 'ワールド冒険の書を編集' : '新しいワールド冒険の書を作成'}</h2>
          </div>
          <button type="button" onClick={() => { playCloseSound(); goBack(); }} onMouseEnter={playHoverSound} className="flex h-8 w-8 items-center justify-center text-slate-300 hover:text-white cursor-pointer" aria-label="閉じる">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex gap-3 px-4 sm:px-5 py-4 border-t border-slate-800 bg-[#0d1627]">
          <button type="button" onClick={() => { playCloseSound(); goBack(); }} onMouseEnter={playHoverSound} disabled={saving} className="flex-1 min-h-[44px] py-2.5 bg-[#1a2333] border border-slate-700 text-slate-300 font-bold hover:bg-slate-800 hover:border-slate-600 disabled:opacity-50 transition-all text-xs sm:text-sm cursor-pointer">キャンセル</button>
          <button type="button" onClick={handleSave} onMouseEnter={playHoverSound} disabled={saving} className="flex-1 min-h-[44px] py-2.5 bg-amber-500 text-slate-950 font-black border-b-2 border-amber-700 shadow-[0_0_16px_rgba(245,158,11,0.2)] hover:bg-amber-400 disabled:opacity-50 transition-all text-xs sm:text-sm cursor-pointer">▶ {saving ? '保存中...' : isEdit ? '更新する' : '作成して開始'}</button>
        </div>
      </div>
      <style>{`
        ...
        @keyframes world-edit-modal-in { from { opacity: 0; transform: translateY(8px) scale(.985); } to { opacity: 1; transform: translateY(0) scale(1); } }
...
function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return <div><label className="block text-xs font-bold text-slate-300 mb-1.5 uppercase tracking-wider">{label}{required && <span className="text-amber-400 ml-1">*</span>}</label>{children}</div>;
}
