...
function WorldCard({ slotNumber, world, meta, onOpen, onEdit, onDelete }: { slotNumber: number; world: WorldWithMembers; meta?: WorldMeta; onOpen: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <article className="save-slot-card selectable-pulse group relative overflow-hidden bg-[#10192d] transition-all duration-200 hover:-translate-y-0.5 hover:border-amber-400 hover:shadow-[0_0_26px_rgba(245,158,11,.20)]">
      ...
      <div className="relative z-10 p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
          ...
          <div className="md:col-span-3 flex justify-end"><button type="button" onClick={onOpen} className="save-slot-load w-full md:w-auto px-6 py-3.5 text-xs sm:text-sm tracking-wider flex items-center justify-center gap-2 font-mono font-bold text-emerald-100 transition-all hover:border-amber-400 hover:text-amber-300 hover:bg-amber-500/10 hover:shadow-[0_0_18px_rgba(245,158,11,.18)] active:scale-[.98]"><span>LOAD</span><ChevronRight className="h-4 w-4" /></button></div>
        </div>
