...
function StyleSelector({ style, generating, resetting, onSelect }: { style: string | null; generating: boolean; resetting: boolean; onSelect: (id: string) => void }) {
  return <div className="mb-6 rounded-sm border-2 border-[#1a2333] bg-[#0d1627] p-4 sm:p-5 shadow-lg"><div className="flex items-center justify-between gap-3 mb-4 pb-3 border-b border-[#1a2333]"><div className="flex items-center gap-2 min-w-0"><span className="text-[#ffb000] font-mono text-xs">▶</span><div><p className="text-xs sm:text-sm font-bold text-[#ffb000] tracking-wide font-mono uppercase">CHRONICLE FORMAT // 旅の書・編纂流派</p><p className="text-[10px] sm:text-xs text-zinc-500 mt-0.5">記録を、どんな世界の物語として残す？</p></div></div><span className="hidden sm:inline text-[10px] text-zinc-600 font-mono">SELECT YOUR CHRONICLE</span></div><div className="grid grid-cols-1 md:grid-cols-3 gap-3">{WIKI_STYLES.map((s) => { const preview = STYLE_PREVIEWS[s.id]; const Icon = preview?.icon ?? BookOpen; const selected = style === s.id; return <button key={s.id} type="button" onClick={() => onSelect(s.id)} disabled={generating || resetting} className={`selectable-pulse ${selected ? 'selectable-pulse-active' : ''} group relative overflow-hidden rounded-sm border-2 p-3.5 text-left transition-all duration-200 ${selected ? 'border-[#ffb000] bg-[#1a2333] shadow-[0_0_18px_rgba(255,176,0,0.22)]' : 'border-[#1a2333] bg-[#050a14] hover:border-[#334155] hover:bg-[#111c30]'}`}><div className="absolute right-[-12px] top-[-18px] text-[#ffb000]/[0.035] transition-transform duration-300 group-hover:scale-110"><Icon size={100} strokeWidth={1} /></div><div className="relative z-10"><div className="flex items-center justify-between gap-2"><span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded-sm border border-[#334155] bg-[#0a1120] text-[#32cd32]">{preview?.eyebrow ?? s.name}</span>{selected && <span className="text-[9px] font-mono font-bold text-[#ffb000]">▶ SELECTED</span>}</div><div className="mt-3 flex items-center gap-2"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-sm border ${selected ? 'border-[#ffb000] bg-[#ffb000]/10 text-[#ffb000]' : 'border-[#334155] bg-[#0d1627] text-zinc-500'}`}><Icon className="w-5 h-5" /></div><div className="min-w-0"><h3 className={`text-sm font-bold truncate ${selected ? 'text-[#ffb000]' : 'text-zinc-200'}`}>{preview?.title ?? s.name}</h3><p className="text-[10px] text-zinc-500 mt-0.5">{preview?.tone ?? s.description}</p></div></div><div className="mt-3 rounded-sm border border-[#1a2333] bg-[#0a1120] p-2.5"><p className="text-[10px] sm:text-[11px] leading-relaxed text-zinc-300 font-serif">{preview?.sample ?? s.description}</p></div><div className="mt-2 flex flex-wrap gap-1.5">{(preview?.badges ?? [s.description]).map((badge) => <span key={badge} className="rounded-sm border border-[#334155] bg-[#111c30] px-1.5 py-0.5 text-[9px] font-mono text-zinc-400">{badge}</span>)}</div></div></button>; })}</div></div>;
}
...
function WikiContent({ world, style, hasArticle, article, generating, resetting, cooldownActive, onGenerate, onReset, onAiTest, locationCount, locations, isGeneratedWikipedia, onOpenLocation }: { world: WorldWithMembers; style: string | null; hasArticle: boolean; article: string | null; generating: boolean; resetting: boolean; cooldownActive: boolean; onGenerate: () => void; onReset: () => void; onAiTest: () => void; locationCount: number; locations: LocationWithPhotos[]; isGeneratedWikipedia: boolean; onOpenLocation?: (locationId: string) => void }) {
  ...
  const pageClass = isWikipedia ? 'bg-[#0d1627] text-zinc-200 border-[#ffb000]' : isScp ? 'bg-[#07141b] text-zinc-200 border-[#22c7ff]' : isAncient ? 'bg-[#160e09] text-[#ead8bf] border-[#ff8a00]' : 'bg-[#0d1627] text-zinc-200 border-[#1a2333]';
  const headerClass = isWikipedia ? 'bg-[#101b2d] border-[#ffb000] text-[#ffb000]' : isScp ? 'bg-[#0a1820] border-[#22c7ff] text-[#22c7ff]' : 'bg-[#1a1009] border-[#ff8a00] text-[#ff8a00]';
