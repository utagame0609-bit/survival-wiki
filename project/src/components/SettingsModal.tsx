...
export function SettingsModal({ onClose, installPrompt, onInstallPromptUsed }: SettingsModalProps) {
  ...
  const handleVolumeChange = (value: number) => { const normalized = setSoundVolume(value); setSoundVolumeState(normalized); void saveUserSoundSettings({ seVolume: normalized, seReverb: Math.round(getStoredReverbAmount() * 100) }).catch((error) => console.error('Failed to save SE volume:', error)); };
  const handleReverbChange = (value: number) => { const normalized = setStoredReverbAmount(value / 100); void saveUserSoundSettings({ seVolume: getSoundVolume(), seReverb: Math.round(normalized * 100) }).catch((error) => console.error('Failed to save SE reverb:', error)); };
