...
export function ScpDossierArticle({ world, locations, content, mainPhotoUrl, narratorLine, locationLinks }: Props) {
  ...
  useEffect(() => {
    ...
    const loadPhotos = async () => {
      ...
      if (!cancelled) { setEvidencePhotos(resolved); setActiveCarouselIdx(0); }
    };
    void loadPhotos().catch(() => { if (!cancelled) setEvidencePhotos([]); });
    return () => { cancelled = true; blobUrls.forEach((url) => URL.revokeObjectURL(url)); };
  }, [sourcePhotos]);
  ...
  const paperProps = {
    dossier, world, coordinateLocation, companions, evidencePhotos, activeCarouselIdx, setActiveCarouselIdx,
    indexOpen, setIndexOpen, declassifiedMode, setDeclassifiedMode, setSelectedPhoto,
  };
      ...
      <div className="bg-[#141416] p-3 sm:p-6 lg:p-8">
        <div className="mx-auto w-full max-w-[96rem] space-y-4 sm:space-y-5">
          ...
          <div className="lg:hidden">
            ...
            <div className="mt-4"><DossierHeaderPaper {...paperProps} /></div>
            <div className="mt-4"><DossierBodyPaper {...paperProps} /></div>
          </div>

          <div className="hidden space-y-5 lg:block">
            <div className="grid w-full grid-cols-12 items-stretch gap-5">
              <div className="col-span-4 flex flex-col xl:col-span-3">
                <ResearcherAside objectClass={dossier.objectClass} narratorLine={narratorLine || narrator.quote} coordinateLocation={coordinateLocation} companions={companions} recordingDate={oldestRecordingDate(locations)} embedded />
              </div>
              <div className="col-span-8 xl:col-span-9"><DossierHeaderPaper {...paperProps} /></div>
            </div>
            <DossierBodyPaper {...paperProps} />
          </div>
...
function ResearcherAside({ objectClass, narratorLine, coordinateLocation, companions, recordingDate, embedded = false }: { objectClass: string; narratorLine: string; coordinateLocation: LocationWithPhotos | null; companions: string[]; recordingDate: string; embedded?: boolean }) {
  return (
    <aside className={`${embedded ? 'flex h-full' : 'hidden lg:flex'} flex-col space-y-4 border border-[#333] bg-[#0d0d0f] p-4 shadow-2xl`}>
      <div><div className="mb-1.5 font-mono text-[10px] uppercase tracking-wider text-[#666]">Researcher ID</div><div className="flex items-center gap-3"><div className="relative h-12 w-12 shrink-0 overflow-hidden border border-[#444] bg-[#222]"><PixelNarrator style="scp" compact /><div className="absolute bottom-0 right-0 h-2.5 w-2.5 border border-[#111] bg-[#00ffcc]" /></div><div className="min-w-0 space-y-0.5 font-mono"><div className="flex items-center gap-1.5 text-sm font-bold tracking-tight text-white">Dr. Ark <span className="text-[9px] font-normal text-[#888]">(特異点観測官)</span></div><div className="text-[9px] font-bold tracking-wider text-[#ff3e3e]">CLEARANCE: LEVEL 5</div><div className="text-[9px] text-[#666]">ID: ARK-994-SOCO</div></div></div></div>
...
function DossierHeaderPaper({ dossier, world, coordinateLocation, companions, declassifiedMode, setDeclassifiedMode }: PaperProps) {
  return (
...
function DossierBodyPaper({ dossier, evidencePhotos, activeCarouselIdx, setActiveCarouselIdx, indexOpen, setIndexOpen, declassifiedMode, setSelectedPhoto }: PaperProps) {
