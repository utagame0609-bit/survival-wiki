...
export function ScpMobileCaseData({ dossier, narratorLine, coordinateLocation, companions, recordingDate, open, onToggle, player, worldName }: Props) {
  return (
    <div className="space-y-2 border border-[#333] bg-[#0f0f12] p-3 font-mono text-xs shadow-md">
      <div className="flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><div className="relative h-8 w-8 shrink-0 overflow-hidden border border-[#444] bg-[#222] [&>div]:!h-8 [&>div]:!w-8 [&>div]:!rounded-none [&>div]:!border-0 [&>div]:!bg-transparent [&_img]:!scale-100"><PixelNarrator style="scp" compact /></div><div className="min-w-0"><div className="truncate text-[11px] font-bold leading-none text-white">Dr. Ark <span className="text-[9px] font-normal text-[#888]">(特異点観測官)</span></div><div className="mt-0.5 text-[9px] font-bold text-[#ff3e3e]">LV-{dossier.securityClearance} // ARK-994</div></div></div><div className="shrink-0 border border-[#ff3e3e] bg-[#1a0a0a] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-[#ff3e3e]">{dossier.objectClass}</div></div>
