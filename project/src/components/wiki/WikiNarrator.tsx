    ...
    <div className={`mt-5 rounded-sm border-2 p-4 sm:p-5 ${narrator.panel} shadow-[0_0_20px_rgba(0,0,0,.25)]`}>
      <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
        <div className="flex sm:flex-col items-center gap-2 sm:w-28 shrink-0">
          ...
          <div className={`text-[9px] sm:text-[10px] font-bold font-mono text-center border px-2 py-1 rounded-sm ${narrator.text} border-current`}>
            {narrator.role}
          </div>
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2 mb-2">
            <div className={`flex items-center gap-2 text-xs sm:text-sm font-bold font-mono ${narrator.text}`}>
              <span className="inline-block h-2 w-2 rounded-full bg-current shadow-[0_0_8px_currentColor]" />【{narrator.name}】
            </div>
