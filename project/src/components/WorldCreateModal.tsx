      ...
      <div className="world-create-modal-panel w-full max-w-lg max-h-[calc(100vh-2rem)] overflow-y-auto rounded-sm bg-[#0a1120] border-2 border-amber-500/80 shadow-[0_0_35px_rgba(245,158,11,0.25)] text-slate-100">
        ...
        <div className="p-5 space-y-4">
          ...
          <Field label="WORLD NAME // ワールド名" required>
            <input autoFocus type="text" value={name} onChange={(event) => setName(event.target.value)} onFocus={playInputFocusSound} placeholder="例: アストリア古王国・忘却の地" className="modal-input text-sm text-slate-100 placeholder-slate-600" />
          </Field>

          <Field label="PLAYER PROFILE // プレイヤー名とアバター">
            <div className="flex items-center gap-3">
              <PhotoPicker file={playerPhoto} onChange={setPlayerPhoto} label="プレイヤー写真" />
              <input type="text" value={player} onChange={(event) => setPlayer(event.target.value)} onFocus={playInputFocusSound} placeholder="あなたの名前 (例: 探索者アルト)" className="modal-input flex-1 text-sm text-slate-100 placeholder-slate-600" />
            </div>
          </Field>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button type="button" onClick={() => { playAddSound(); setMembers([...members, { name: '', photo: null }]); }} onMouseEnter={playHoverSound} className="flex items-center gap-1 text-xs font-bold text-emerald-400 hover:text-emerald-300 transition-colors uppercase tracking-wider cursor-pointer"><Plus className="w-3.5 h-3.5" />スロット追加</button>
            </div>
            <div className="space-y-2">
              {members.map((member, index) => (
                <div key={index} className="flex items-center gap-2 bg-[#090d16] p-2 rounded-sm border border-slate-800">
                  ...
                  <input type="text" value={member.name} onChange={(event) => { const next = [...members]; next[index] = { ...next[index], name: event.target.value }; setMembers(next); }} onFocus={playInputFocusSound} placeholder={`メンバー${index + 1} (例: リリス)`} className="modal-input flex-1 text-xs" />
                  {members.length > 1 && <button type="button" onClick={() => { playDeleteSound(); setMembers(members.filter((_, i) => i !== index)); }} onMouseEnter={playHoverSound} className="w-8 h-8 rounded-sm text-slate-500 hover:text-rose-400 hover:bg-rose-950/30 flex items-center justify-center transition-colors cursor-pointer" aria-label="メンバーを削除"><X className="w-4 h-4" /></button>}
                </div>
              ))}
            </div>
          </div>
          ...
          <Field label="WORLD MEMO // 探検概要・目標">
            <textarea value={memo} onChange={(event) => setMemo(event.target.value)} onFocus={playInputFocusSound} placeholder="ワールドの概要、難易度、攻略目標など" rows={3} className="modal-input resize-none text-xs text-slate-200" />
          </Field>
        </div>

        <div className="flex gap-3 px-5 py-4 border-t border-slate-800 bg-[#0d1627]">
          <button type="button" onClick={() => { playCloseSound(); onClose(); }} onMouseEnter={playHoverSound} disabled={saving} className="flex-1 py-2.5 rounded-sm bg-[#1a2333] border border-slate-700 text-slate-300 font-bold hover:bg-slate-800 hover:border-slate-600 disabled:opacity-50 transition-all text-xs uppercase tracking-wider cursor-pointer">キャンセル</button>
          <button type="button" onClick={handleSave} onMouseEnter={playHoverSound} disabled={saving} className="flex-1 py-2.5 rounded-sm bg-amber-500 text-slate-950 font-black border border-amber-400 shadow-[0_0_16px_rgba(245,158,11,0.25)] hover:bg-amber-400 disabled:opacity-50 transition-all text-xs uppercase tracking-wider cursor-pointer">{saving ? 'GENERATING... // 作成中' : '▶ 作成して開始 (START)'}</button>
        </div>
      </div>
...
function PhotoPicker({ file, onChange, label }: { file: File | null; onChange: (file: File | null) => void; label: string }) {
  ...
  return (
    <label onMouseEnter={playHoverSound} className="relative h-11 w-14 shrink-0 cursor-pointer overflow-hidden rounded-sm border border-slate-700 bg-[#050a14] flex items-center justify-center text-slate-500 hover:border-amber-500 hover:text-amber-400 transition-colors" title={label}>
