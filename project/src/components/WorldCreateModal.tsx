...
export function WorldCreateModal({
  ...
  return (
    ...
    >
      <div className="world-create-modal-panel flex w-full max-w-lg max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] flex-col overflow-hidden border-2 border-amber-500 bg-[#0a1120] text-slate-100 shadow-[0_0_35px_rgba(245,158,11,0.25)]">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-800 bg-[#0d1627] px-4 sm:px-5 py-3.5">
          ...
          <button type="button" onClick={() => { playModalCloseSound(); onClose(); }} onMouseEnter={playHoverSound} className="flex h-8 w-8 items-center justify-center text-slate-400 hover:text-white cursor-pointer" aria-label="閉じる">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          ...
          <Field label="WORLD NAME // ワールド名" required>
            <input autoFocus type="text" value={name} onChange={(event) => setName(event.target.value)} onFocus={playInputFocusSound} placeholder="例: アストリア古王国・忘却の地" className="modal-input text-sm" />
          </Field>
          ...
          <section className="border border-slate-700/80 bg-[#0a101d] p-3 space-y-3">
            ...
            <div className="flex items-center gap-3">
              <PhotoPicker previewUrl={playerPreview} onChange={setPlayerPhotoFile} label="プレイヤー写真" accent="amber" size="large" />
              <input type="text" value={player} onChange={(event) => setPlayer(event.target.value)} onFocus={playInputFocusSound} placeholder="あなたの名前 (例: 探索者アルト)" className="modal-input flex-1 text-sm" />
              <label className="hidden sm:flex min-h-[42px] shrink-0 items-center gap-1.5 border border-slate-600 bg-slate-800 px-2.5 text-[11px] font-mono font-bold text-amber-300 cursor-pointer hover:border-amber-400 transition-colors">
                <Camera className="h-3.5 w-3.5" />写真変更
          ...
          <section className="border border-slate-700/80 bg-[#0a101d] p-3 space-y-2.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {namedMembers.map((member, index) => <PartyChip key={`${member.name}-${index}`} name={member.name} photoUrl={member.previewUrl} />)}
            </div>
            ...
            <div className="border border-slate-700/80 bg-[#12192c] p-2.5 space-y-2">
              ...
              {members.map((member, index) => (
                <div key={index} className="flex items-center gap-2">
                  <PhotoPicker previewUrl={member.previewUrl} onChange={(file) => setMemberPhotoFile(index, file)} label={`メンバー${index + 1}写真`} accent="cyan" />
                  <input type="text" value={member.name} onChange={(event) => setMembers((current) => current.map((item, i) => i === index ? { ...item, name: event.target.value } : item))} onFocus={playInputFocusSound} placeholder={`メンバー${index + 1} (例: リリス)`} className="modal-input flex-1 text-xs" />
                  {members.length > 1 && <button type="button" onClick={() => removeMember(index)} onMouseEnter={playHoverSound} className="flex h-10 w-10 shrink-0 items-center justify-center border border-slate-700 bg-[#141824] text-slate-400 hover:border-rose-500 hover:text-rose-300 transition-colors cursor-pointer" aria-label="メンバーを削除"><X className="h-4 w-4" /></button>}
                </div>
              ))}

              <button type="button" onClick={addMember} onMouseEnter={playHoverSound} className="flex min-h-[38px] w-full items-center justify-center gap-1 border border-cyan-500/60 bg-cyan-950/20 px-3 text-xs font-black text-cyan-300 hover:border-cyan-400 hover:bg-cyan-950/40 transition-colors cursor-pointer">
                <Plus className="h-4 w-4" />スロット追加
              </button>
            </div>
          </section>
        ...
        <div className="flex shrink-0 gap-3 border-t border-slate-800 bg-[#0d1627] px-4 sm:px-5 py-4">
          <button type="button" onClick={() => { playCloseSound(); onClose(); }} onMouseEnter={playHoverSound} disabled={saving} className="flex-1 min-h-[44px] border border-slate-700 bg-[#1a2333] py-2.5 text-xs sm:text-sm font-bold text-slate-300 hover:bg-slate-800 hover:border-slate-600 disabled:opacity-50 transition-all cursor-pointer">キャンセル</button>
          <button type="button" onClick={handleSave} onMouseEnter={playHoverSound} disabled={saving} className="flex-1 min-h-[44px] border-b-2 border-amber-700 bg-amber-500 py-2.5 text-xs sm:text-sm font-black text-slate-950 shadow-[0_0_16px_rgba(245,158,11,0.2)] hover:bg-amber-400 disabled:opacity-50 transition-all cursor-pointer">▶ {saving ? '作成中...' : '作成して開始 (START)'}</button>
        </div>
      </div>
...
function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return <div><label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-300">{label}{required && <span className="ml-1 text-rose-400">*</span>}</label>{children}</div>;
}
