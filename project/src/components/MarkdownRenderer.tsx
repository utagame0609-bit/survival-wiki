...
function inlineMarkdown(text: string, locationLinks: LocationLink[] = []): ReactNode[] {
  const locationNames = locationLinks.map((location) => location.name).filter(Boolean).sort((a, b) => b.length - a.length);
  ...
  return parts.map((part, index) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) return <code key={index} className="rounded bg-stone-100 px-1 py-0.5 text-[0.9em]">{part.slice(1, -1)}</code>;
    const location = locationLinks.find((item) => item.name === part);
    if (location) return <button key={index} type="button" onClick={location.onClick} className="text-[#36c] hover:underline">{part}</button>;
...
export function MarkdownRenderer({ content, className = '', locationLinks = [] }: Props) {
  ...
  const flushParagraph = () => {
    ...
    if (text) blocks.push(<p key={`p-${blocks.length}`} className="my-3">{inlineMarkdown(text, locationLinks)}</p>);
    paragraph = [];
  };

  const flushList = () => {
    if (listItems.length === 0) return;
    blocks.push(<ul key={`ul-${blocks.length}`} className="my-4 list-disc space-y-1 pl-6">{listItems.map((item, index) => <li key={index}>{inlineMarkdown(item, locationLinks)}</li>)}</ul>);
    listItems = [];
  };
  ...
  lines.forEach((line, index) => {
    ...
    if (/^#{1,3}\s+/.test(trimmed)) {
      ...
      if (level === 1) blocks.push(<h1 key={`h-${index}`} className={`${common} my-5 border-b border-[#a2a9b1] pb-2 text-2xl sm:text-3xl`}>{inlineMarkdown(text, locationLinks)}</h1>);
      else { headings.push({ level: level as 2 | 3, text, id }); if (level === 2) blocks.push(<h2 id={id} key={`h-${index}`} className={`${common} mt-8 mb-3 border-b border-[#a2a9b1] pb-1 text-xl sm:text-2xl`}>{inlineMarkdown(text, locationLinks)}</h2>); else blocks.push(<h3 id={id} key={`h-${index}`} className={`${common} mt-6 mb-2 text-lg sm:text-xl font-semibold`}>{inlineMarkdown(text, locationLinks)}</h3>); }
      return;
    }
    if (/^---+$/.test(trimmed)) { flushAll(); blocks.push(<hr key={`hr-${index}`} className="my-6 border-0 border-t border-[#a2a9b1]" />); return; }
    if (/^[-*]\s+/.test(trimmed)) { flushParagraph(); listItems.push(trimmed.replace(/^[-*]\s+/, '')); return; }
  ...
  const tableOfContents = headings.length >= 2 ? <nav aria-label="目次" className="my-6 border border-[#a2a9b1] bg-[#f8f9fa] p-4 text-sm"><div className="mb-2 font-semibold text-[#202122]">目次</div><ol className="space-y-1">{headings.map((heading, index) => <li key={heading.id} className={heading.level === 3 ? 'ml-5' : ''}><button type="button" onClick={() => document.getElementById(heading.id)?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="text-left text-[#36c] hover:underline">{index + 1}. {heading.text}</button></li>)}</ol></nav> : null;
