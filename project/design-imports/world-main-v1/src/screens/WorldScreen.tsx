...
import { BookOpen, Clock3, FileText, MapPin, X, Compass, Sparkles } from 'lucide-react';
...
export function WorldScreen({ worldId, worldName, navigate, goBack }: { worldId: string; worldName: string; navigate: NavigateFn; goBack: () => void }) {
  ...
  return (
    <div className="min-h-screen overflow-y-scroll [scrollbar-gutter:stable] bg-[#0a1120] text-[#e2e8f0] flex flex-col font-dot relative crt-scanlines">
      ...
      {!loading && world && (
        <>
          {/* Elegant Dark Command Nav Bar */}
          <nav className="sticky top-18 sm:top-20 z-20 bg-[#0d1627] border-b-2 border-[#1a2333] shadow-lg">
                ...
                {tabs.map((t, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`command-btn h-11 sm:h-12 px-3.5 sm:px-5 flex items-center gap-2.5 rounded-sm text-xs sm:text-sm font-bold select-none transition-all ${
                        isActive
          ...
          {/* Wiki Location Detail Modal - Elegant Dark Window */}
          {wikiLocation && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-dot">
              ...
              <div className="relative z-10 w-full max-w-lg max-h-[85vh] overflow-hidden rounded-sm bg-[#0a1120] text-[#e2e8f0] border-4 border-double border-[#ffb000] shadow-[0_0_25px_rgba(255,176,0,0.2),inset_0_0_10px_rgba(255,176,0,0.1)] flex flex-col motion-safe:animate-[wiki-modal-enter_180ms_cubic-bezier(.22,.8,.35,1)]">
                ...
                <div className="p-3 bg-[#0d1627] border-t-2 border-[#1a2333] flex justify-end">
                  <button
                    onClick={handleCloseWikiLocation}
                    className="command-btn px-4 py-2 bg-[#1a2333] text-[#ffb000] text-xs font-bold font-mono"
                  >
                    閉じる (ESC)
                  </button>
                </div>
