...
import { Activity, ArrowUpDown, Crown, Footprints, MapPin, Compass, Sparkles, Clock } from 'lucide-react';
...
export function TimelineTab({ world, reloadKey }: { world: WorldWithMembers; reloadKey: number }) {
  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-4xl mx-auto font-dot">
      <style>{`
      ...
      {/* World Log Status Banner (Elegant Dark Header) */}
      <div className="w-full mb-6 rounded-sm border-2 border-[#1a2333] bg-[#0d1627] p-4 px-5 shadow-lg flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h2 className="text-xs font-bold tracking-widest text-[#ffb000] uppercase font-mono whitespace-nowrap">
          ...
          <div className="flex items-center gap-3 text-[11px] text-zinc-400 font-mono pl-4">
            <span className="flex items-center gap-1 whitespace-nowrap">
              <Activity size={12} className="text-[#32cd32]" />
              総記録数: <strong className="text-[#32cd32]">{totalRecords}</strong>
            </span>
            <span className="text-zinc-700">•</span>
            <span className="whitespace-nowrap">
              経過日数: <strong className="text-[#ffb000]">{totalDays} Days</strong>
            </span>
          </div>
        </div>
        <div className="self-end sm:self-auto shrink-0">
          <button
            ...
            className="command-btn flex items-center gap-1.5 rounded-sm bg-[#1a2333] px-3.5 py-2 text-xs font-bold text-zinc-200"
          >
      ...
      {!loading && groups.length === 0 && (
        <EmptyState message="タイムラインがありません。ロケーションを記録すると冒険ルートが自動生成されます。" />
      )}

      {!loading && groups.length > 0 && (
        <>
          <div ref={timelineRef} className="relative">
            {/* Retro dotted vertical track line */}
            ...
            {/* Glowing active progression trail */}
            <div
              className="pointer-events-none absolute left-[7px] top-[18px] z-10 w-[3px] origin-top rounded-full bg-gradient-to-b from-[#ffb000] via-[#ffb000] to-[#32cd32] shadow-[0_0_12px_#ffb000] transition-[height] duration-500 ease-out"
            ...
            >
              <div
                className={`relative flex items-center justify-center w-7 h-7 rounded-full text-[#0a1120] shadow-lg ring-2 ring-[#0a1120] ${
                  activeMilestone
                    ? 'bg-[#ffb000] shadow-[0_0_15px_#ffb000]'
                    : 'bg-[#32cd32] shadow-[0_0_12px_#32cd32]'
              ...
              >
                {isRevealingMilestone && (
                  <span
                    className="milestone-burst pointer-events-none absolute inset-0 rounded-full border-2 border-[#ffb000]"
                ...
                {activeMilestone && (
                  <span
                    className={`absolute left-8 whitespace-nowrap rounded-sm bg-[#ffb000] px-2 py-0.5 text-[10px] font-bold text-[#0a1120] shadow-md border border-white/40 ${
...
function DayChapter({
  ...
  return (
    <section ref={onRef} className="relative scroll-mt-24">
      {/* Day Header Button - RPG Stage/Day Banner */}
      <button
        type="button"
        onClick={onSelect}
        className={`group relative w-full min-h-[80px] mb-3 overflow-hidden rounded-sm border-2 text-left transition-all duration-200 ${
          isActive
            ? 'border-[#ffb000] bg-[#0d1627] shadow-[0_0_20px_rgba(255,176,0,0.2)]'
            : 'border-[#1a2333] bg-[#0d1627] hover:border-[#334155] hover:bg-[#111c30]'
        }`}
      ...
      >
        {group.bgPhotoPath && (
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <TimelinePhoto
              ...
              alt=""
              className="absolute inset-0 w-full h-full object-cover opacity-[0.12] [mask-image:linear-gradient(to_right,black_0%,black_60%,transparent_100%)] [-webkit-mask-image:linear-gradient(to_right,black_0%,black_60%,transparent_100%)]"
            />
          </div>
        )}
        <div className="relative z-10 flex min-h-[80px] items-center gap-3.5 px-4 py-3 sm:px-5">
          {/* Day Badge */}
          <div
            className={`flex h-10 w-14 shrink-0 items-center justify-center rounded-sm border-2 font-mono ${
              isActive
                ? 'border-[#ffb000] bg-[#ffb000] text-[#0a1120] font-black shadow-md'
                : 'border-[#334155] bg-[#1a2333] text-zinc-400 font-bold'
            }`}
          >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className={`text-sm sm:text-base font-bold ${isActive ? 'text-[#ffb000]' : 'text-zinc-200'}`}>
              ...
              {isMilestoneUnlocked && milestone && (
                <span
                  className={`inline-flex items-center gap-1 rounded-sm border px-2 py-0.5 text-[10px] font-bold ${
                    isActive
                      ? 'border-[#ffb000] bg-[#ffb000] text-[#0a1120] shadow-sm'
                      : 'border-[#ffb000]/50 bg-[#ffb000]/20 text-[#ffb000]'
                  }`}
                >
            ...
            <div className="mt-1 flex items-center gap-2 text-xs">
              <span className={`font-mono ${isExpanded ? 'text-[#32cd32]' : 'text-zinc-400'}`}>
                {group.locations.length}件の冒険ログ
              </span>
            </div>
          </div>
        </div>
...
function TimelineEntry({ loc }: { key?: string | number; loc: LocationWithPhotos }) {
    ...
    return (
      <article className="relative overflow-hidden rounded-sm border-2 border-[#1a2333] bg-[#0d1627] shadow-md hover:border-[#334155] transition-colors">
        <div className="grid grid-cols-1 md:grid-cols-[minmax(150px,0.8fr)_minmax(240px,1.7fr)]">
          {hasPhoto ? (
            <div className="w-full h-44 md:h-full min-h-44 overflow-hidden bg-[#050a14] border-b md:border-b-0 md:border-r border-[#1a2333]">
              <TimelinePhoto storagePath={mainPhoto!.storage_path} alt={loc.name} className="w-full h-full object-cover" />
            </div>
          ) : (
            <div className="h-24 md:h-full min-h-24 bg-[#050a14] flex items-center justify-center text-zinc-600 border-b md:border-b-0 md:border-r border-[#1a2333]">
          ...
          <div className="p-4 min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h4 className="text-sm sm:text-base font-bold text-[#ffb000] break-words flex items-center gap-1.5">
                  <span className="text-[#ffb000]">◆</span>
                  <span>{loc.name}</span>
                </h4>
                <p className="text-xs text-[#32cd32] font-mono mt-1">
            ...
            {hasMemo && (
              <p className="mt-2.5 text-xs sm:text-sm leading-relaxed text-zinc-300 whitespace-pre-wrap break-words p-3 rounded-sm bg-[#050a14] border border-[#1a2333] font-mono">
                {loc.detail_memo}
              </p>
            )}
  ...
  return (
    <div className="relative min-h-9 flex items-center gap-3 text-xs sm:text-sm px-3.5 py-2.5 rounded-sm bg-[#0d1627] border border-[#1a2333]">
      <span className="text-[#ffb000] font-mono">▸</span>
      <span className="min-w-0 font-bold text-zinc-200 truncate">{loc.name}</span>
      <span className="shrink-0 text-xs text-[#32cd32] font-mono">X:{loc.x} Y:{loc.y} Z:{loc.z}</span>
      <span className="ml-auto shrink-0 text-xs text-zinc-500 font-mono">{time}</span>
    </div>
  );
}
