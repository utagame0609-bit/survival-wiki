...
import { playModalOpenSound, playModalCloseSound } from '@/lib/sound';
...
export function ChestModal({ collectionItems, onClose, onOpenLocation }: ChestModalProps) {
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md text-slate-100 overflow-y-auto font-mono flex items-center justify-center p-3 sm:p-6">
      ...
      <div className="relative z-10 w-full max-w-4xl max-h-[calc(100vh-32px)] overflow-hidden rounded-sm bg-[#0a1120] border-2 border-amber-500/80 shadow-[0_0_35px_rgba(245,158,11,0.25)] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-800 bg-[#0d1627] flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <span
              className="relative block w-5 h-4 rounded-[1px] border border-amber-500 bg-amber-600 shadow-[inset_0_1px_0_rgba(255,255,255,0.2)]"
              aria-hidden="true"
            >
          ...
          <button
            ...
            aria-label="コレクションを閉じる"
            className="w-8 h-8 flex items-center justify-center rounded-sm text-slate-400 hover:bg-[#1a2333] hover:text-amber-400 transition-colors border border-transparent hover:border-slate-700 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto overscroll-contain p-4 sm:p-6">
          {collectionItems.length === 0 ? (
            <EmptyState message="まだ探検記録写真がありません。ロケーション作成時に写真を記録するとここに保管されます。" />
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
              {collectionItems.map((item, idx) => (
                <CollectionSlot
                  key={`${item.location.id}-${item.storagePath}-${idx}`}
                  slotNumber={idx + 1}
                  item={item}
                  onOpen={() => {
                    playModalOpenSound();
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-slate-800 bg-[#0d1627] flex-shrink-0 text-xs text-slate-400">
          ...
          <button
            type="button"
            onClick={() => { playModalCloseSound(); onClose(); }}
            className="px-4 py-1.5 rounded-sm bg-[#1a2333] border border-slate-700 text-slate-300 hover:text-white hover:border-slate-500 font-bold text-xs uppercase cursor-pointer"
          >
            閉じる (CLOSE)
          </button>
        </div>
...
function CollectionSlot({ item, slotNumber, onOpen }: { item: CollectionItem; slotNumber: number; onOpen: () => void; key?: string | number }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group relative aspect-square rounded-sm border border-slate-800 bg-[#090d16] p-1 text-left shadow-md hover:border-amber-500 hover:shadow-[0_0_20px_rgba(245,158,11,0.2)] active:scale-[0.98] transition-all cursor-pointer overflow-hidden flex flex-col"
    >
      <div className="relative w-full h-full overflow-hidden rounded-sm bg-[#050a14] border border-slate-800">
        {src ? (
          ...
          <div className="w-full h-full bg-[#050a14] flex items-center justify-center text-slate-600">
