...
import { Plus, X, Camera, Globe } from 'lucide-react';
      ...
      <div className="world-create-modal-panel w-full max-w-lg max-h-[calc(100vh-2rem)] overflow-y-auto rounded-sm bg-[#0a1120] border-2 border-amber-500/80 shadow-[0_0_35px_rgba(245,158,11,0.25)] text-slate-100">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-[#0d1627]">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-sm bg-amber-500 text-slate-950 flex items-center justify-center font-bold text-xs shadow-[0_0_10px_rgba(245,158,11,0.4)]">
              W
            </div>
            <div>
          ...
          <button
            ...
            aria-label="閉じる"
            className="w-8 h-8 rounded-sm text-slate-400 hover:text-amber-400 hover:bg-[#1a2333] flex items-center justify-center transition-colors border border-transparent hover:border-slate-700 cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <Field label="PLAYER PROFILE // プレイヤー名とアバター">
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="modal-input flex-1 text-sm text-slate-100 placeholder-slate-600"
              />
            </div>
          </Field>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                COMPANION MEMBERS // 関連同行メンバー
              </label>
              <button
                type="button"
                onClick={() => setMembers([...members, { name: '', photo: null }])}
                className="flex items-center gap-1 text-xs font-bold text-emerald-400 hover:text-emerald-300 transition-colors uppercase tracking-wider cursor-pointer"
              >
                ...
                <div key={index} className="flex items-center gap-2 bg-[#090d16] p-2 rounded-sm border border-slate-800">
                  ...
                  {members.length > 1 && (
                    <button
                      ...
                      className="w-8 h-8 rounded-sm text-slate-500 hover:text-rose-400 hover:bg-rose-950/30 flex items-center justify-center transition-colors cursor-pointer"
                      aria-label="メンバーを削除"
                    >
                      <X className="w-4 h-4" />
                    </button>
          ...
          <Field label="WORLD MEMO // 探検概要・目標">
            <textarea
              ...
              rows={3}
              className="modal-input resize-none text-xs text-slate-200"
            />
          </Field>
        </div>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-slate-800 bg-[#0d1627]">
          <button
            ...
            onClick={() => { playCloseSound(); onClose(); }}
            disabled={saving}
            className="flex-1 py-2.5 rounded-sm bg-[#1a2333] border border-slate-700 text-slate-300 font-bold hover:bg-slate-800 hover:border-slate-600 disabled:opacity-50 transition-all text-xs uppercase tracking-wider cursor-pointer"
          >
            キャンセル
          </button>
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-2.5 rounded-sm bg-amber-500 text-slate-950 font-black border border-amber-400 shadow-[0_0_16px_rgba(245,158,11,0.25)] hover:bg-amber-400 disabled:opacity-50 transition-all text-xs uppercase tracking-wider cursor-pointer"
          >
            {saving ? 'GENERATING... // 作成中' : '▶ 作成して開始 (START)'}
          </button>
        </div>
      </div>
...
function PhotoPicker({ file, onChange, label }: { file: File | null; onChange: (file: File | null) => void; label: string }) {
  ...
  return (
    <label
      className="relative h-11 w-14 shrink-0 cursor-pointer overflow-hidden rounded-sm border border-slate-700 bg-[#050a14] flex items-center justify-center text-slate-500 hover:border-amber-500 hover:text-amber-400 transition-colors"
      title={label}
    >
      {preview ? (
