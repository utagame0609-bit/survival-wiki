...
export function ChestModal({ collectionItems, onClose, onOpenLocation }: ChestModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-sm font-sans"
      onMouseDown={(e) => {
    ...
    >
      <div className="relative w-full max-w-3xl max-h-[90vh] bg-[#1e2330] border-2 border-amber-500/70 shadow-[0_0_40px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden motion-safe:animate-[modal-enter_180ms_cubic-bezier(.22,.8,.35,1)]">
        ...
        <div className="flex items-center justify-between px-4 sm:px-5 py-3.5 bg-[#161a24] border-b-2 border-[#2d3548]">
          <div className="flex items-center gap-3">
            <span
              aria-hidden="true"
              className="relative block w-6 h-5 border-2 border-amber-400 bg-amber-600 shadow-[0_0_10px_rgba(245,158,11,0.5)]"
            >
        ...
        <div className="px-4 sm:px-5 py-3 bg-[#161a24] border-t-2 border-[#2d3548] flex justify-between items-center text-xs text-slate-400 font-mono">
          ...
          <button
            ...
            className="min-h-[38px] px-4 py-1.5 bg-[#141824] text-slate-200 hover:text-amber-400 border border-slate-700 hover:border-amber-500 font-bold text-xs cursor-pointer"
          >
            閉じる
          </button>
        </div>
      ...
      {selectedPhoto && (
        <div
          className="fixed inset-0 z-60 flex items-center justify-center p-3 sm:p-4 bg-black/90 backdrop-blur-md font-sans"
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) {
              playCancelSound();
            ...
            <div className="p-4">
              ...
              <div className="mt-3 p-3 bg-[#141824] border border-[#2d3548] text-xs font-mono">
                <div className="flex items-center justify-between text-emerald-400 font-bold">
                  ...
                  <span className="text-slate-400">{new Date(selectedPhoto.location.created_at).toLocaleDateString('ja-JP')}</span>
                </div>
