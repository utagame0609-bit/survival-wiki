...
export function WorldCreateScreen({ gameId, gameName, worldId, navigate, goBack }: { gameId: string; gameName: string; worldId?: string; navigate: NavigateFn; goBack: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onMouseDown={(event) => {
      ...
      <div className="world-edit-modal-panel w-full max-w-lg max-h-[calc(100vh-2rem)] overflow-y-auto bg-[#0a1120] border-2 border-amber-500/60 shadow-[0_0_24px_rgba(245,158,11,0.15),0_20px_50px_rgba(0,0,0,0.7)] font-mono">
        <div className="flex items-center px-5 py-4 border-b border-[#1a2333] bg-[#0d1627]"><div className="flex items-center gap-3"><span className="font-mono text-[10px] px-2 py-0.5 border border-amber-500/50 bg-amber-500/10 text-amber-400 font-bold">WORLD CONFIG</span><h2 className="text-base font-bold text-slate-100">{isEdit ? 'ワールドを編集' : 'ワールドを追加'}</h2></div></div>
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-[#1a2333] bg-[#0d1627]"><button type="button" onClick={() => { playCloseSound(); goBack(); }} disabled={saving} className="flex-1 py-2.5 border border-slate-700 bg-[#070c18] text-slate-300 font-bold hover:border-slate-500 hover:text-white disabled:opacity-50 transition-all cursor-pointer">キャンセル</button><button type="button" onClick={handleSave} disabled={saving} className="flex-1 py-2.5 bg-amber-500 text-black font-bold border-b-2 border-amber-700 shadow-[0_2px_8px_rgba(245,158,11,0.25)] hover:bg-amber-400 active:border-b-0 disabled:opacity-50 transition-all cursor-pointer">▶ {saving ? '保存中...' : isEdit ? '更新する' : '追加する'}</button></div>
      </div>
      <style>{`.modal-input { width: 100%; padding: 0.55rem 0.75rem; border: 1px solid rgb(51 65 85); background: #070c18; color: rgb(226 232 240); outline: none; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; font-size: 0.8125rem; transition: border-color 160ms ease, box-shadow 160ms ease, background 160ms ease; }.modal-input::placeholder { color: rgb(71 85 105); }.modal-input:focus { border-color: #f59e0b; box-shadow: 0 0 0 1px #f59e0b, 0 0 10px rgba(245,158,11,0.2); }.world-edit-modal-panel { animation: world-edit-modal-in 180ms cubic-bezier(.22,.8,.35,1) both; transform-origin: center; }@keyframes world-edit-modal-in { from { opacity: 0; transform: translateY(8px) scale(0.985); } to { opacity: 1; transform: translateY(0) scale(1); } }@media (prefers-reduced-motion: reduce) { .world-edit-modal-panel { animation: none; } }`}</style>
    </div>
  );
}
